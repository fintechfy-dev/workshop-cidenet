...
5. Commitea con un mensaje que identifique la iteración (ej. `feat: iteración 2 — reglas de negocio de usuarios`). El hook de pre-commit va a correr `dotnet format` + tests automáticamente y bloquea el commit si algo falla — si eso pasa, corrige antes de reintentar, no fuerces el commit.
